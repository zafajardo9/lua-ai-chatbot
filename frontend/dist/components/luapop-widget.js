...
const publicEnv = {
    ...
    cdnUrl: process.env.NEXT_PUBLIC_LUA_CDN_URL ??
...
function getWidgetConfig() {
    ...
    const isVoiceSafeHost = typeof window !== "undefined" &&
    ...
    return {
        ...
        chatWindowWidth: typeof window !== "undefined" && window.innerWidth < 640
            ? "calc(100vw - 20px)"
            : "380px",
        chatWindowHeight: typeof window !== "undefined" && window.innerWidth < 640
            ? "min(78vh, 640px)"
            : "620px",
    ...
    useEffect(() => {
        if (missingVars.length > 0) {
            if (process.env.NODE_ENV !== "production") {
                console.error(`Lua widget disabled. Missing environment variables: ${missingVars.join(", ")}.`);
            }
            return;
        }
        let widgetInstance;
        const existingScript = document.querySelector('script[data-lua-pop="true"]');
...
export function LuaWidgetConfigWarning() {
    ...
    return (<div className="config-warning" role="alert">
